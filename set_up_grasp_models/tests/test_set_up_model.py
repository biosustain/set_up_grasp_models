...
class TestSetUpModel(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.file_in_stoic = os.path.join(self.test_folder, 'putida_with_PPP_plaintext.txt')

    def test_set_up_model_empty_base(self):

        true_res = pd.read_excel(os.path.join(self.test_folder, 'true_res_putida_v1.xlsx'), sheet_name=None)
    ...
    def test_set_up_model_not_empty_base(self):
        ...
        true_res = pd.read_excel(os.path.join(self.test_folder, 'true_res_putida_v2.xlsx'), sheet_name=None)

        general_file = os.path.join(self.test_folder, 'putida_v1_manual2_EX.xlsx')
        model_name = 'putida_v2'
    ...
    def test_set_up_model_not_empty_base_mets_file(self):
        ...
        true_res = pd.read_excel(os.path.join(self.test_folder, 'true_res_putida_v4.xlsx'), sheet_name=None)

        general_file = os.path.join(self.test_folder, 'putida_v1_manual2_EX.xlsx')
        file_in_mets_conc = os.path.join(self.test_folder, 'met_concs.xlsx')
        model_name = 'putida_v4'
        file_out = os.path.join(self.test_folder, model_name + '.xlsx')
