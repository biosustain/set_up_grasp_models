...
def set_up_proteomics(rxn_order, ex_rxns, file_in_prot_ranges, map_prot_abbreviation, time_point, replicate, prot_lb = 0.99, prot_ub=1.01):
    ...
    prot_data_df = pd.DataFrame(data=np.matrix([lb, mean, ub]).transpose(), index=list(in_rxns), columns=['lower_bound', 'mean', 'upper_bound'])
    ...
    # to be deleted
    prot_ranges.columns = ['lower_bound', 'mean', 'upper_bound']
