...
def check_flux_balance(data_dict: dict) -> bool:
    ...
    if len(stoic_df.index) == len(flux_df.index):
        ...
        fixed_mets = set(mets_df.loc[mets_df['fixed?'].eq(1), 'fixed?'].index.values)
        ...
        for met in met_in_rxns.keys():
            ...
            if flux_balance != 0 and met in balanced_mets:
                print('The flux for', met, 'is not balanced. The difference in flux is', flux_balance)
                flag = True
            elif flux_balance == 0 and met not in balanced_mets:
                print(met, 'should be in balanced mets')
                flag = True
...
def check_balanced_metabolites(data_dict: dict) -> bool:
    ...
    :param data_dict (dict): a dictionary that represents the excel file with the GRASP model.
    :return: flag (bool)
    """
